...
public:
    ...
    TokenType op;
    BinNode() = default;
    BinNode(const std::shared_ptr<Node> &_l, const std::shared_ptr<Node> &_r, const TokenType &_op) : l(_l), r(_r), op(_op) {}
...
public:
    ...
    FnDeclNode() = default;
    FnDeclNode(decltype(return_type) &_ret_type, std::string &_name) : return_type(std::move(_ret_type)), name(std::move(_name))
    {
    }
...
IOType get_io_type(const std::string &_iostr)
{
    if (_iostr == "putchar")
    {
